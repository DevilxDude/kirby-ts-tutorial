...
async function gameSetup() {
	...
	const { map: level1Layout, spawnPoints: level1SpanwPoints } = await makeMap(
		k,
		"level-1"
	);
	...
	k.scene("level-1", () => {
		...
		const kirb = makePlayer(
			k,
			level1SpanwPoints.player[0].x,
			level1SpanwPoints.player[0].y
		);
		...
		for (const flame of level1SpanwPoints.flame) {
			makeFlameEnemy(k, flame.x, flame.y);
		}

		for (const guy of level1SpanwPoints.guy) {
			makeGuyEnemy(k, guy.x, guy.y);
		}

		for (const bird of level1SpanwPoints.bird) {
